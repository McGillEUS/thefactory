...
export default function WeekView(props: {
  ...
  function getSlotHeight(startTime: Date, endTime: Date): number {
    ...
    const oneHourHeight = 90; // Base height for 1 hour
  ...
  return (
    <div
      className="flex justify-center items-center mx-auto py-4 mt-24 lg:mt-16 mb-20 "
      style={{ height: props.calendarHeight }}
    >
        ...
        {Object.keys(props.officeHours).map((day, dayIndex) => (
          <div key={dayIndex} className="flex flex-col">
            ...
            {props.officeHours[day].map((officeHour, index) => (
              <div
                key={index}
                className="bg-factory-green text-white rounded-md lg:p-2 pt-1 m-1 cursor-pointer transition-transform transform hover:scale-105 text-center"
                style={{
              ...
              >
                <div>{officeHour.attributes.First_Name}</div>
